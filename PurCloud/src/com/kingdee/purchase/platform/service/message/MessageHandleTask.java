...
public class MessageHandleTask {
	...
	private static void handleQuotationMessage(MessageInfo messageInfo){
		...
		RequestContext supRequestContext = MessageUtils.getRequestContext(messageInfo,
				"com.kingdee.purchase.openapi", "supplier.getSupplier", "1");
		...
		try{
			...
			QuotationInfo quoInfo = JSONUtil.json2QuotationInfo(jsonObject, true);
			
			
			params.clear();
			params.put("memberId", quoInfo.getSupplierMemberId());
			supRequestContext.setBussinessParams(params);
			
			jsonObject = apiProcressor.processApiService(supRequestContext);
			SupplierInfo supplierInfo = JSONUtil.jsonSupplierInfo(jsonObject);
			supplierInfo.setDestId(quoInfo.getSupplierMemberId());
			
			SpringContextUtils.getBean(ISupplierService.class).saveSupplier(supplierInfo);
