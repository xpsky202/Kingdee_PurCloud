...
import com.kingdee.purchase.openapi.client.ErrorHandler;
...
public class RequestPolicy implements Cloneable {
    ...
    private Protocol            requestProtocol          = Protocol.param;
    private Protocol            responseProtocol         = requestProtocol;
    private boolean             responseCompress         = true;
    private int                 requestCompressThreshold = -1;                   // 默认不开启
    private int                 timeout                  = 5000;                 // 5秒
    private HttpMethodPolicy    httpMethod               = HttpMethodPolicy.AUTO;
    private String              queryStringCharset       = "GB18030";            // Request URL query string encoder
    ...
    public static RequestPolicy getAuthPolicy() {
        if (authPolicy == null) {
            authPolicy = new RequestPolicy().setHttpMethod(HttpMethodPolicy.POST).setRequestProtocol(Protocol.http).setUseHttps(true).setNeedAuthorization(false);
        }
        return authPolicy;
    }
